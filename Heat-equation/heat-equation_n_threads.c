...
void apply_stencil(float* data, size_t width, size_t height, size_t offset, float alpha, int t) {
    #pragma omp parallel for num_threads(t)
    for (size_t x = 1; x < width -1 ; x ++){ 
        size_t lim = 1+ ((x+offset)%2); 
        for (size_t y = lim;  y < (height-1) ; y+=2){ 
...
float compute_delta(float* data, float* prev, size_t width, size_t height, int t ) {
    float res = 0.0 ; 
    #pragma omp parallel for reduction(+:res) num_threads(t)
    for (size_t x = 0; x<width; x++){
        for (size_t y = 0 ; y<height; y++){
            res += fabsf(prev[pos(width,x,y)] - data[pos(width,x,y)]); 
...
void run_simulation(size_t width, size_t height, size_t steps, const char* filename, int t) {
    ...
    for(; n < steps; n++) {
        ...
        apply_stencil(data, width, height, n % 2, 0.2f, t);
...
int main(int argc, char** argv) {
    ...
    for (int t = 1; t<(16); t++){
        bef = seconds(); 
        run_simulation((size_t)width, (size_t)height, (size_t)steps, filename, t);
        after = seconds();
        printf("'%f', ", after-bef); 
    }
